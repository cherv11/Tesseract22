...
А ещё не пишите `print` внутри функции! Кто знает, пригодится вам значение для вывода на экран или ещё куда-то?
### Напишем парочку задач ❓
...
### Задачи ❓
1. Напишите класс для представления ученика.  
Атрибуты: имя, фамилия, цифра (класс в смысле год обучения) буква и оценки (словарь, в котором ключи — это предметы, а значения — массивы с оценками.  
Методы: поставить оценку X по предмету Y, вывести средний балл по предмету X, `__str__` для вывода информации об ученике в виде: `Жулебина Анастасия 10Г, средний балл по всем предметам: 4,45`

2. Создайте словарь с учениками со структурой:
```py
{8:
    {'A': 
        {"Ученик 1", "Ученик 2"},
...
```
Сделайте четыре класса, два восьмых и два девятых, в каждом из которых по три ученика. 
Напишите функцию, которая поставит всем ученикам в определённом классе одну и ту же оценку по заданному предмету. Аргументы: класс, буква, предмет, оценка.
